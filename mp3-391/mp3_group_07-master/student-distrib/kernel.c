...
#include "keyboard.h"       //for keyboard support
#include "rtc.h"            //for rtc support
...
void entry(unsigned long magic, unsigned long addr) {
    ...
    if (CHECK_FLAG(mbi->flags, 3)) {
        ...
        while (mod_count < mbi->mods_count) {
			boot_blk = (boot_block*) ((unsigned int)mod->mod_start);
            end = (int*) (unsigned int)mod->mod_end;
    ...
    // Enable interrupt for IRQ1 (Keyboard)
    enable_irq(1);
	...
	terminal_map();
    initialize_terminal(0);
    initialize_terminal(1);
    initialize_terminal(2);
	...
	int32_t exec_rval;
	exec_rval = execute((uint8_t*)"shell");
	//exec_rval = execute((uint8_t*)"shell");
	//exec_rval = execute((uint8_t*)"shell");
